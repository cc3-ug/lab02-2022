...
def check_ex3():
    try:
        ...
        if v.count == 0 and r.count == 0:
            return (0, utils.failed('[vector_set] don\'t forget to call free or use realloc... ¯\\_(⊙︿⊙)_/¯'), '')
        # run tests
...
def lab2_c_mm():
    ...
    if len(not_found) == 0:
        ...
        errors += '\n' + utils.create_error('LFSR', ex2_result[2])
        errors += '\n' + utils.create_error('vector', ex3_result[2])
        errors = errors.strip()
        grade = 0
        grade += ex1_result[0]
        grade += ex2_result[0]
